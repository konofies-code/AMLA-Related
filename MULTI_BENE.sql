...
BEGIN
UPDATE AML_CUSTOMER SET IDENTIFICATION_NUMBER = '' WHERE IDENTIFICATION_NUMBER = '0'
END
...
SELECT A.ACCOUNTID,B.TXNID,C.CUSTOMERID,ROW_NUMBER() OVER(PARTITION BY A.ACCOUNTID,B.TXNID ORDER BY A.ACCOUNTID) COUNTS FROM AML_ACCOUNT A
INNER JOIN (

select BENEFICIARY_ACCOUNTNO,TXNID from AML_TRANSACTION 
WHERE LTRIM(RTRIM(BENEFICIARY_ACCOUNTNO)) IN (SELECT ACCOUNTNUMBER FROM AML_ACCOUNT) AND SOURCE = 'CASA' AND TXNTYPE <> 'COCKD'
AND CONVERT(DATE,IMPORTED_DATE) = CONVERT(DATE,GETDATE())

) B ON A.ACCOUNTNUMBER = LTRIM(RTRIM(B.BENEFICIARY_ACCOUNTNO))
INNER JOIN AML_CUSTOMER_ACCOUNT C ON A.ACCOUNTID = C.ACCOUNTID
...
--INSERT INTO AML_TRANSACTION_BENEFICIARY
--(TXNID
--,BENEFICIARYNAME1
--,BENEFICIARYNAME2
--,BENEFICIARYNAME3
...
--,CUST.MIDDLENAME
--,CUST.LASTNAME
--,CUST.PRESENTADDRESS1
--,CUST.PRESENTADDRESS2
--,CUST.PRESENTADDRESS3
--,CUST.CUSTOMERNO
--,ACCT.ACCOUNTNUMBER
--,CUST.DATEOFBIRTH
--,CUST.PLACEOFBIRTH
--,CUST.IDENTIFICATION_TYPE
--,CUST.IDENTIFICATION_NUMBER
--,CUST.NATUREOFBUSINESS
--,CASE WHEN CUST.INDIVIDUAL_CORPORATE = 1  THEN 'N' WHEN CUST.INDIVIDUAL_CORPORATE = 2 THEN 'Y' END
--FROM AML_TRANSACTION ATM
--INNER JOIN (SELECT * FROM (
--SELECT A.ACCOUNTID,B.TXNID,C.CUSTOMERID,ROW_NUMBER() OVER(PARTITION BY A.ACCOUNTID,B.TXNID ORDER BY A.ACCOUNTID) COUNTS FROM AML_ACCOUNT A
--INNER JOIN (
...
BEGIN
UPDATE AML_TRANSACTION SET CPNAME1 = BENEFICIARYNAME1
,CPNAME2 = BENEFICIARYNAME2
,CPNAME3 = BENEFICIARYNAME3
WHERE  TXNTYPE  = 'COCKD' AND CONVERT(DATE,IMPORTED_DATE) = CONVERT(DATE,GETDATE()) AND SOURCE = 'CASA'
AND ((LEN(BENEFICIARYNAME1) = 12 and isnumeric(BENEFICIARYNAME1) = 0) or LEN(BENEFICIARYNAME1) <> 12)
END

--SELECT * FROM AML_TRANSACTION WHERE TXNTYPE  = 'COCKD' AND CONVERT(DATE,IMPORTED_DATE) = CONVERT(DATE,GETDATE())
--AND ((LEN(BENEFICIARYNAME1) = 12 and isnumeric(BENEFICIARYNAME1) = 0) or LEN(BENEFICIARYNAME1) <> 12)
...
UPDATE ATM
SET 
cpaccountno = BENEFICIARY_ACCOUNTNO,
...
BEGIN
UPDATE AML_TRANSACTION SET cpaccountno = BENEFICIARY_ACCOUNTNO, BENEFICIARY_ACCOUNTNO = NULL
WHERE  TXNTYPE  = 'COCKD' AND  BENEFICIARY_ACCOUNTNO is not NULL and CONVERT(DATE,IMPORTED_DATE) = CONVERT(DATE,GETDATE())  AND SOURCE = 'CASA'
END
...
----------------------------------------2019-06-29
BEGIN
update AML_TRANSACTION set  
 BENEFICIARY_CUSTOMERREFNO = NULL
...
(
	SELECT DISTINCT TRAN_CODE FROM AML_REGULATORY_REPORT_MANDATORY BB
	INNER JOIN AML_FIELD CC ON BB.FIELD_ID = CC.FIELD_ID
	WHERE CC.DATASET_NAME = 'CTR1_Beneficiary'
		AND (FIELD_NAME = 'B_LASTNAME' OR FIELD_NAME = 'B_ACCOUNTNO' OR FIELD_NAME = 'B_FIRSTNAME')
) 
AND TXNTYPE in (select distinct TRANSACTION_TYPE from AML_CTR_TRANTYPE)
...
UPDATE AML_TRANSACTION SET BENEFICIARY_ACCOUNTNO = '240180104'
WHERE CONVERT(DATE,IMPORTED_DATE) = CONVERT(DATE,GETDATE()) AND TXNTYPE = 'CTRIA'
AND LTRIM(RTRIM(BENEFICIARYNAME1)) = 'Transaction From/To Bank GL Account'  AND SOURCE = 'CASA'
/***********SET THE DEFAULT BENEACCOUNTNUMBER FROM Transaction From/To Bank GL Account TO 240180104 ******************/
END

Begin

UPDATE AML_TRANSACTION SET TXN_DATE_TIME=  TXNDATE   WHERE  CONVERT(DATE,IMPORTED_DATE) = CONVERT(DATE,GETDATE()) 
UPDATE AML_TRANSACTION SET txndate = CONVERT(DATE,TXNDATE) WHERE  CONVERT(DATE,IMPORTED_DATE) = CONVERT(DATE,GETDATE()) 
END

begin

UPDATE A SET TXNTYPE = 
CASE WHEN ACCOUNTTYPE IN ('901','902','903','904','906','907','908','909','911','912','913','914') THEN 'DTDYK'
 ELSE'CTRIA'END
...
BEGIN
update a set txntype = 'DTDPM' FROM AML_TRANSACTION a left join ref_treasury_accounts b on a.ACCOUNTNUMBER =b.ACCOUNTNO
...
BEGIN
update a set txntype = 'DTDPD' FROM AML_TRANSACTION a left join ref_treasury_accounts b on a.ACCOUNTNUMBER =b.ACCOUNTNO
WHERE --ACCOUNTNUMBER = '111120240242'
...
begin
delete a from aml_transaction a
left join aml_account b on a.AccountNumber = b.ACCOUNTNUMBER
...
BEGIN
UPDATE AML_TRANSACTION SET TXNTYPE = 'DTDYM' WHERE TXNTYPE = 'DTDPM'
END
...
select a.CUSTOMERID, a.ACCOUNTID ,
 ...
 ,getdate()
 from aml_account a
left join Aml_customer_account ca
on a.ACCOUNTID = ca.ACCOUNTID and a.CUSTOMERID = ca.CUSTOMERID
where ca.JOINTID is null
end


BEGIN
UPDATE  A SET TXNTYPE = 'DTDYW'FROM AML_TRANSACTION A
LEFT JOIN AML_ACCOUNT B ON A.AccountNumber = B.ACCOUNTNUMBER
        ...
        FROM 
          ...
          INNER JOIN (
            ...
            WHERE 
              ...
              AND CONVERT(DATE, IMPORTED_DATE) = CONVERT(
                DATE, 
                GETDATE()
              )
          ) B ON A.ACCOUNTNUMBER = LTRIM(
            RTRIM(B.BENEFICIARY_ACCOUNTNO)
          ) 
          INNER JOIN AML_CUSTOMER_ACCOUNT C ON A.ACCOUNTID = C.ACCOUNTID 
        GROUP BY 
          A.ACCOUNTID, 
...
Set
BENEFICIARY_NAME_FLAG = CASE WHEN BENEFICIARY_ACCOUNTNO IS NULL THEN 'Y'
ELSE
BENEFICIARY_NAME_FLAG
END
Where SOURCE = 'CASA' AND TXNTYPE IN ('CPMD', 'CTRIA', 'CCMC') 
AND CONVERT(DATE, IMPORTED_DATE) = CONVERT(DATE, GETDATE())
END
/********************************************************************/
...
Set
BENEFICIARY_ACCOUNTNO = NULL
Where SOURCE = 'CASA' AND TXNTYPE = 'CPMD'
AND CONVERT(DATE, IMPORTED_DATE) = CONVERT(DATE, GETDATE())
END
